...
use Maginium\Framework\Support\Facades\OpenAI;
 ...
 * and providing solutions for errors in the Maginium Integration .
 */
class AiSolution implements Solution
{
     ...
     *
     * @return array An array of documentation links.
    ...
    private function retrieveSolutionFromCache(): mixed
    {
        return Cache::remember(
            ...
            fn() => $this->generateSolution(), // Generate a new solution if not cached
        );
    }
    ...
    private function generateSolution(): string
    {
        ...
        ])->choices[0]->text; // Return the generated text from the first choice
    }
    ...
    private function generatePrompt(Throwable $throwable): string
    {
        ...
        // Get a code snippet from the application frame
        ...
        //     'snippet' => collect($snippet)->map(fn($line, $number) => $number . ' ' . $line)->join(PHP_EOL), // Format the snippet
        //     'file' => $applicationFrame->file, // Pass the file name
        //     'line' => $applicationFrame->lineNumber, // Pass the line number
        //     'exception' => $throwable->getMessage(), // Pass the exception message
        // ]);
